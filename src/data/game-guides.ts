...
const gameGuides: Record<string, GameGuide> = {
  ...
  "dots-boxes": {
    ...
    setup: [
      "Escolha modo contra maquina ou dois jogadores locais.",
      "Escolha tamanho do tabuleiro (4x4 a 7x7 pontos) antes de iniciar a partida.",
  ...
  connections: {
    objective:
      "Conexões é um quebra-cabeça de associação em tempo real com foco em inferência, leitura de pistas e eliminação lógica. Na prática, cada rodada seleciona 16 palavras distribuídas em 4 conjuntos escondidos com exatamente 4 itens em cada conjunto. O desafio é descobrir quais quatro palavras compartilham uma única lógica e enviá-las juntas. A lógica do jogo é parecida com um teste de semântica visual: cada palavra pode parecer óbvia para mais de um tema, e a boa parte da jogada não está em memorizar uma lista, e sim em reduzir incertezas com prova social e exclusão cruzada. O objetivo final é fechar os quatro grupos com menos erros, manter consistência de raciocínio e não se precipitar em padrões muito fáceis que escondem armadilhas. O sistema embaralha os itens a cada rodada para impedir treino por padrão de posição; isso força o jogador a reconstruir o contexto inteiro sempre do zero. O jogo também ajuda quem gosta de jogos de repertório porque exige construir uma taxonomia mental rápida: cor, naipes, categorias técnicas, famílias de objeto, funções, estados, gêneros e até relações de oposição. Mesmo sem dicas diretas, a análise de borda já é útil: uma palavra que parece encaixar em dois conceitos quase sempre é pista de armadilha, então ela ganha mais valor quando isolada. Quando o grupo está certo, o jogo mostra o nome da categoria, o que permite aprender padrões de similaridade sem que seja uma solução óbvia. Em partidas mais rápidas, o ganho vem de antecipar combinações fortes e confiar no critério de exclusão. Em partidas longas, a vantagem está em mapear ambiguidades, testar hipóteses e revisar suposições antes do envio. É um modo excelente para treino cognitivo de classificação e para jogar em sessão curta, porque cada erro custa pouco em tempo e não trava o jogo, apenas reduz sua margem de acerto. ",
    setup: [
      "A rodada carrega 4 grupos diferentes, cada um com 4 palavras, totalizando 16 itens distintos sem repetição.",
      "As palavras aparecem totalmente embaralhadas e não há indicador de grupo, então o começo da rodada é uma leitura de superfície seguida de dedução.",
      "O jogo sorteia as combinações a cada novo tabuleiro, então memorizar posição anterior não resolve; quem resolve de fato olha os conceitos por trás dos termos.",
      "Antes de responder, é possível montar hipóteses parciais só com leitura visual e sem clicar em 4 cartas de uma vez, porque cada seleção parcial já ajuda a enxergar onde a ambiguidade está.",
      "Uma rodada termina quando os 4 grupos são encontrados.",
      "Há repetição de temas ao longo das partidas, mas em ordens variáveis; isso garante treino progressivo sem ficar previsível.",
      "Se um grupo é encontrado, ele sai da área ativa e o restante se fecha visualmente, reduzindo o ruído para as próximas decisões.",
      "Use o histórico do que já foi encontrado para reduzir o espaço de busca; a mecânica não esconde a validade da sua escolha anterior, ela só te empilha vantagem de certeza.",
      "Quando ficar em dúvida, avance em etapas: descarte relações muito amplas (como 'coisas legais' ou 'objetos') e procure definições mais fortes (como 'naipes' ou 'partes de um mesmo conjunto técnico').",
      "No começo do jogo, priorize estabilidade: encontre primeiro um grupo com vínculo explícito, depois avance para os menos óbvios.",
      "Se fizer um erro, a partida continua, mas a janela de risco diminui e o jogo começa a cobrar consistência nas próximas escolhas.",
      "A solução de uma rodada não se altera durante a sessão; uma escolha inválida não muda o painel de palavras, apenas força uma revisão estratégica.",
    ],
    steps: [
      "Leia a lista completa de 16 palavras e anote mentalmente possíveis eixos de semelhança: classe gramatical, função, material, cor, posição, gênero, lugar de origem, categoria comercial, parte do jogo etc.",
      "Escolha 4 palavras que tenham a menor chance de conflito semântico e envie apenas quando o vínculo parecer mais do que coincidência superficial.",
      "Se não houver confiança, guarde a tentativa e faça uma segunda camada de filtragem comparando pares já eliminados para diminuir ruído.",
      "Quando decidir por um grupo, envie e observe se o nome revelado bate com a sua hipótese; isso valida se a suposição de categoria está correta.",
      "Ao falhar, não corrija com a mesma lógica; mude o eixo de classificação e tente outra abordagem (ex.: de tema geral para relação técnica).",
      "Repita até achar 4 grupos inteiros, sempre reequilibrando risco e evidência.",
      "Evite pressionar em cima de uma palavra ambígua para caber em quatro caminhos ao mesmo tempo, porque isso costuma gerar grupos incorretos mesmo em rodadas fáceis.",
      "Depois de dois erros, reavalie tudo: normalmente sobram 2 ou 3 palavras em categorias com menos contraste, e aí o método de eliminação ganha força.",
      "Quando uma tentativa é aceita, remapeie o restante do conjunto: palavras que antes pareciam distantes podem agora ficar mais claras por exclusão.",
      "Conduza o jogo como uma prova: confirme com evidência e só então finalize o envio.",
    ],
    rules: [
      "Cada palavra pertence a exatamente um grupo e não pode ser reutilizada em outro conjunto.",
      "Cada envio precisa ter exatamente 4 palavras; seleção fora desse tamanho não é validada.",
      "Os grupos são sorteados aleatoriamente por rodada a partir de um banco de categorias variado, então você não pode depender de padrão repetitivo de aparência.",
      "Palavras com relação muito ampla não garantem vitória: 'formas parecidas', 'objetos', 'coisas' e termos genéricos devem ser tratados com cautela.",
      "Há grupos com vínculo funcional (ex.: naipes, cores), geográfico, semântico, esportivo, anatômico, técnico e linguístico, portanto o jogo mistura níveis de abstração.",
      "Não existe pontuação ocultando resposta correta no meio do caminho: tudo é jogado pela associação que você constrói em tempo real.",
      "Quando um grupo é encontrado, suas palavras deixam de estar disponíveis na grade e ficam registradas para evitar confusão visual.",
      "A validação confere o conjunto inteiro de 4 itens; não há pontuação parcial por 1 ou 2 acertos dentro do envio.",
      "Grupos errados não mudam o conjunto de palavras, então a informação principal ainda está no que sobrou para ser descoberto.",
    ],
    controls: [
      "Clique ou toque em cada palavra para montar uma seleção de até 4 itens.",
      "Envie apenas quando estiver com quatro palavras alinhadas por um critério único e consistente.",
      "A seleção parcial é útil para testar o encadeamento mental antes do envio final.",
      "Use a mensagem de estado para acompanhar erros e o avanço de grupos encontrados.",
      "Recomece a rodada quando quiser uma nova combinação sorteada.",
    ],
    visual: [
      "Palavras selecionadas ficam destacadas com cor de ação para reduzir cliques errados.",
      "Grupos concluídos aparecem com o nome da categoria e o grupo correspondente de itens.",
      "Contador de erros acompanha sua margem de confiança e ajuda a decidir até onde avançar.",
      "A seleção parcial de palavras permite perceber conflitos de categoria antes de confirmar.",
      "A lista embaralhada e a ausência de dicas visuais exigem leitura de padrão e comparação entre candidatos.",
    ],
    tips: [
      "Comece pelos conjuntos mais densos em semântica, como naipes, cores, estações, linguagens de programação, meios de transporte, partes do corpo e esportes.",
      "Se uma palavra parece encaixar em dois campos, mantenha-a em observação e feche esse ramo depois de eliminar alternativas exclusivas.",
      "Evite enviar por sensação. Dúvidas não resolvidas viram erro, e erro repetido reduz sua margem de decisão final.",
      "Use uma estratégia de camadas: primeiro tema explícito, depois tema implícito, por fim tema reverso (opostos, contrastes, pares ou funções complementares).",
      "Quando duas palavras fortes parecem fazer parte de grupos diferentes, procure uma estrutura de oposição: positivo/negativo, origem/uso, material/estado, parte do corpo/região ou direção/posição no espaço.",
      "O nome do grupo revelado é recurso de aprendizado; revise mentalmente por que aquelas quatro palavras se conectam para melhorar sua próxima rodada.",
      "Rodadas com categorias como 'geografia', 'tecnologia' e 'esportes' costumam exigir mais precisão lexical; não generalize.",
      "Se estiver travado, anote mentalmente quais palavras já não cabem mais em nenhuma hipótese segura e descarte sem medo.",
      "Em jogos longos, reduza o custo de processamento: escolha uma proposta principal, valide com consistência e então avance para um grupo com menos ambiguidade.",
      "Uma seleção de 4 itens errados costuma indicar que você misturou nível de similaridade: gênero lexical em vez de relação funcional, ou função prática em vez de tema principal.",
      "A ordem de resolução não importa, mas a ordem estratégica importa; termine primeiro os grupos óbvios para ganhar clareza no conjunto restante.",
      "Não subestime as categorias de 'pegadinha': um item de aparência parecida pode ser o principal indicador de que sua hipótese ainda está fraca.",
      "Ao terminar, compare a lista completa para checar se algum grupo estava implícito demais; isso acelera muito no próximo round.",
      "Quem joga com calma quase sempre ganha de quem tenta fazer tudo em 30 segundos porque as relações ambíguas não desaparecem com pressa.",
      "Trate o erro como dado de treinamento: cada falha elimina uma taxonomia e aumenta a precisão do próximo palpite.",
      "Procure padrões de linguagem dos dados: abreviações, objetos de marca, categorias de jogos, natureza, direção, geografia e classes de material ajudam a descobrir os temas repetidos do banco.",
      "A melhor forma de aprender é jogar em modo observação e só depois acelerar: revise mentalmente cada grupo correto e escreva a razão de pertencimento antes da próxima partida.",
      "Se repetir erro no mesmo tipo de categoria, é sinal de que a heurística de leitura precisa subir de camada semântica.",
      "Conexões é sobre classificação mental robusta: não vence quem acha 4 palavras iguais, vence quem identifica a regra comum mais forte em menos tempo.",
    ],
  },
  "word-search": {
