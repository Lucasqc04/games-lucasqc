...
function Button({
  ...
  const tones = {
    default: "border border-white/10 bg-white/10 text-slate-100 hover:border-brand-400 hover:bg-brand-500/15",
...
function Select<T extends string>({
  ...
  return (
    <label className="flex min-w-[9rem] flex-col gap-1 text-[0.67rem] font-black uppercase tracking-normal text-amber-100/80">
      {label}
      <select
        value={value}
        onChange={(event) => onChange(event.target.value as T)}
        className="h-10 rounded-md border border-white/10 bg-slate-950 px-2 text-sm font-bold normal-case text-white outline-none transition focus:border-brand-400"
      >
        {options.map((option) => (
...
function ReadingCardButton({
  ...
  return (
    <button
      ...
      className={cx(
        "group min-w-0 rounded-md border bg-slate-950/70 p-1.5 text-left transition hover:-translate-y-0.5 hover:border-brand-400",
        selected ? "border-brand-400 shadow-[0_0_0_2px_rgba(255,106,0,0.28)]" : "border-white/10",
      )}
    >
      <span className="mb-1 block truncate px-1 text-[0.62rem] font-black uppercase leading-tight text-amber-100/80">{entry.position}</span>
      <img
        ...
        draggable={false}
      />
      <span className="mt-1 block truncate px-1 text-center text-[0.68rem] font-bold text-slate-100">
    ...
    <article className="space-y-3">
      <div className="grid grid-cols-[5.5rem_minmax(0,1fr)] gap-3">
        ...
        <div className="min-w-0">
          <p className="text-xs font-black uppercase text-brand-300">Carta {card.id}</p>
          <h2 className="text-xl font-black leading-tight text-white">{card.name}</h2>
          <p className="mt-2 text-sm leading-relaxed text-slate-300">{card.essence}</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-1.5">
        {card.keywords.map((keyword) => (
          <span key={keyword} className="rounded-md border border-brand-400/30 bg-brand-500/10 px-2 py-1 text-xs font-bold text-amber-100">
            {keyword}
          </span>
        ))}
      </div>
      <div className="grid gap-2 text-sm leading-relaxed text-slate-300">
        <p>
          <b className="text-white">Na área escolhida:</b> {card.areas[focus]}
        </p>
        <p>
          <b className="text-white">Luz:</b> {card.light}
        </p>
        <p>
          <b className="text-white">Sombra:</b> {card.shadow}
        </p>
        <p>
          <b className="text-white">Conselho:</b> {card.advice}
        </p>
        <p>
          <b className="text-white">Tempo:</b> {card.timing}
        </p>
        <p>
          <b className="text-white">Sim/não:</b> {card.yesNo}
        </p>
      </div>
      <div className="grid gap-2">
        <h3 className="text-sm font-black uppercase text-amber-100">Áreas da vida</h3>
        {FOCUS_AREAS.map((area) => (
          <p key={area.id} className="rounded-md bg-white/[0.055] px-3 py-2 text-sm leading-relaxed text-slate-300">
            <b className="text-white">{area.label}:</b> {card.areas[area.id]}
          </p>
        ))}
      </div>
      <div>
        <h3 className="mb-2 text-sm font-black uppercase text-amber-100">Combinações úteis</h3>
        <ul className="grid gap-1.5 text-sm leading-relaxed text-slate-300">
          {card.combinations.map((combination) => (
            <li key={combination} className="rounded-md bg-white/[0.055] px-3 py-2">
              {combination}
            </li>
          ))}
        </ul>
...
function Guide() {
  return (
    <details className="rounded-md border border-white/10 bg-white/[0.045] p-3 text-sm leading-relaxed text-slate-300" open>
      <summary className="cursor-pointer text-sm font-black uppercase text-white">Como jogar, passo a passo</summary>
...
export function BaralhoCigano({ record, stats }: Props) {
  ...
  return (
    <div className="min-h-[calc(100svh-9rem)] rounded-md border border-slate-900 bg-[radial-gradient(circle_at_50%_0%,rgba(255,106,0,0.23),transparent_35%),linear-gradient(135deg,#111827,#050505_55%,#1b1307)] p-3 text-slate-100 shadow-inner sm:p-4">
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_26rem]">
        <main className="min-w-0 space-y-3">
          <div className="flex flex-wrap items-end gap-2 rounded-md border border-white/10 bg-black/35 p-2">
            <Select
              label="Tiragem"
              value={spreadId}
            ...
            <label className="min-w-[13rem] flex-1 text-[0.67rem] font-black uppercase tracking-normal text-amber-100/80">
              Pergunta
              <input
                value={question}
                onChange={(event) => setQuestion(event.target.value)}
                placeholder="Ex.: O que preciso saber sobre este trabalho?"
                className="mt-1 h-10 w-full rounded-md border border-white/10 bg-slate-950 px-3 text-sm font-semibold normal-case text-white outline-none transition placeholder:text-slate-500 focus:border-brand-400"
              />
            </label>
          ...
          <div className="grid gap-2 rounded-md border border-white/10 bg-black/30 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-xs font-black uppercase text-brand-300">{spread.title}</p>
                <p className="text-sm leading-relaxed text-slate-300">{spread.description}</p>
              </div>
              <div className="flex gap-2 text-xs font-black uppercase text-amber-100/80">
                <span>{reading.length}/{spread.count} cartas</span>
                <span>{stats.plays} leituras salvas</span>
              </div>
            </div>
            <p className="rounded-md border border-amber-300/15 bg-amber-300/10 px-3 py-2 text-sm font-bold text-amber-100">{status}</p>
            ...
            {reading.length ? (
              ...
              <div className="grid min-h-[18rem] place-items-center rounded-md border border-dashed border-white/15 bg-black/25 p-6 text-center">
                <div>
                  ...
                  <p className="text-lg font-black text-white">Mesa pronta para o sorteio</p>
          ...
          {drawMode === "manual" ? (
            <section className="rounded-md border border-white/10 bg-black/30 p-3">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <div>
                  <h2 className="text-sm font-black uppercase text-white">Baralho fechado</h2>
                  <p className="text-xs text-slate-400">Toque em uma carta para abrir. As escolhidas ficam visíveis na grade.</p>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={cx(
                        ...
                        picked ? "border-brand-400 bg-brand-500/15" : "border-white/10 bg-slate-950/80",
                      )}
                    >
                      <img
        ...
        <aside className="min-h-0 rounded-md border border-white/10 bg-black/45 p-3 xl:max-h-[calc(100svh-11rem)] xl:overflow-y-auto">
          <div className="mb-3 grid grid-cols-2 gap-1 rounded-md bg-white/[0.055] p-1">
            <button
              type="button"
              onClick={() => setPanel("reading")}
              className={cx("rounded px-3 py-2 text-sm font-black transition", panel === "reading" ? "bg-brand-500 text-black" : "text-slate-300 hover:bg-white/10")}
            >
              Leitura
            </button>
            <button
              ...
              className={cx("rounded px-3 py-2 text-sm font-black transition", panel === "wiki" ? "bg-brand-500 text-black" : "text-slate-300 hover:bg-white/10")}
            >
              Wiki
            </button>
          </div>
          ...
          {panel === "reading" ? (
            <div className="space-y-3">
              <section className="rounded-md border border-white/10 bg-white/[0.045] p-3">
                <h2 className="text-sm font-black uppercase text-white">Síntese da tiragem</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-300">{readingSummary}</p>
              </section>
            ...
            <div className="space-y-3">
              <label className="block text-[0.67rem] font-black uppercase text-amber-100/80">
              ...
              <div className="grid max-h-56 grid-cols-2 gap-1.5 overflow-y-auto pr-1">
                {wikiCards.map((card) => (
                  <button
                    ...
                    className={cx(
                      "grid grid-cols-[2.4rem_minmax(0,1fr)] items-center gap-2 rounded-md border p-1 text-left transition hover:border-brand-400 hover:bg-white/10",
                      selectedCardId === card.id ? "border-brand-400 bg-brand-500/10" : "border-white/10 bg-white/[0.035]",
                    )}
                  >
                    <img src={card.image} alt="" className="aspect-[512/736] w-full rounded object-fill" draggable={false} />
                    <span className="min-w-0">
                      <span className="block truncate text-xs font-black text-white">{card.name}</span>
                      <span className="block truncate text-[0.68rem] text-slate-400">{card.keywords.slice(0, 2).join(", ")}</span>
                    </span>
                  </button>
                ))}
              </div>
